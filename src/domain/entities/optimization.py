...
class OptimizationResult(BaseModel):
    ...
    Cost figures are per day. Solvers that price the peak report baseline/optimized cost as
    energy cost plus the daily share of the monthly maximum-demand charge, with the split in
    the optional breakdown fields below (None when a solver does not report it).
    """
    time_slots: List[str]
    ...
    energy_savings_lkr: Optional[float] = None
    demand_charge_savings_lkr: Optional[float] = Field(
        default=None, description="Daily share (1/30) of the monthly LKR/kVA charge saved by lowering the peak"
    )
